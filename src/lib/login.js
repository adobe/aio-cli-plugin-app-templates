...
/**
 * Login to AIO CLI and generate new access token
 *
...
async function retrieveAccessToken () {
  ...
  try {
    ...
    const token = await getToken(ctx, { open: true })
    return token
  } catch (err) {
    throw new Error(`Cannot get token for context: ${err.message}`)
  }
}
